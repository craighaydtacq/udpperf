...
if __name__ == "__main__":
    ...
    parser.add_argument("-a", "--address", default=UDP_IP, help=f"IPv4 address to TX to (default: {UDP_IP})")
    parser.add_argument(
        "-p", "--port", default=UDP_PORT, type=int, help=f"UDP port to TX to (default: {UDP_PORT})"
    )
    parser.add_argument("--ssb", required=True, type=int, help="The size of a single sample in bytes.")
    parser.add_argument("--spp", required=True, type=int, help="The number of samples to bundle into one UDP packet.")
    parser.add_argument("--max-udp-size", default=JUMBO_FRAME_MAX_SIZE, type=int, help=f"Max allowed UDP payload size (default: {JUMBO_FRAME_MAX_SIZE})")
    parser.add_argument(
        "-s",
        "--packet_size",
        default=PACKET_SIZE,
    ...
    try:
        ...
        with open(args.file, 'rb') as f:
            while True:
                ...
                print(f"\rSent packet #{packet_count} ({len(chunk)} bytes). Total sent: {total_bytes_sent} bytes.", end="")
